...
public class SendSMS implements Runnable {
	...
	public void run() {
		...
		// Construct data
		try {
			String data = smsApiUrl + "/api/mt/SendSMS?" + "APIKey=" + URLEncoder.encode(smsApiKey, "UTF-8") + "&senderid="
					+ URLEncoder.encode(senderId, "UTF-8") + "&channel=" + URLEncoder.encode(channel, "UTF-8") + "&DCS="
					+ URLEncoder.encode(dcs, "UTF-8") + "&flashsms=" + URLEncoder.encode(flashsms, "UTF-8") + "&number="
					+ URLEncoder.encode(toMobileNumber, "UTF-8") + "&text=" + URLEncoder.encode(smsMessage, "UTF-8")
